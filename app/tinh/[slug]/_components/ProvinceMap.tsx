'use client'

import { useEffect, useRef } from 'react'
import dynamic               from 'next/dynamic'

// Province centroids
const PROVINCE_COORDS: Record<number, [number, number]> = {
  1:  [21.03, 105.85],  4:  [22.40, 103.97],  6:  [22.32, 104.35],
  8:  [22.04, 105.83],  10: [21.68, 105.65],  14: [21.29, 105.17],
  15: [22.02, 105.97],  17: [21.85, 106.76],  19: [21.00, 106.19],
  20: [21.09, 106.56],  22: [21.32, 106.23],  24: [20.47, 105.90],
  25: [20.25, 105.97],  26: [20.54, 106.08],  27: [20.82, 106.06],
  28: [20.71, 105.74],  29: [20.91, 105.97],  30: [20.86, 106.69],
  31: [20.02, 106.06],  33: [19.81, 105.79],  34: [18.68, 105.70],
  35: [17.99, 106.22],  38: [17.50, 106.55],  40: [16.75, 107.19],
  48: [16.05, 108.22],  49: [15.57, 108.47],  51: [15.12, 108.80],
  52: [14.17, 109.04],  54: [13.09, 109.30],  56: [12.25, 109.15],
  58: [11.92, 108.44],  60: [11.09, 108.11],  62: [13.80, 108.12],
  64: [14.35, 108.00],  66: [12.65, 108.05],  67: [12.27, 107.60],
  68: [11.60, 108.12],  70: [11.37, 106.90],  72: [11.00, 106.67],
  74: [10.95, 106.83],  75: [10.95, 107.28],  77: [10.40, 107.14],
  79: [10.83, 106.63],  80: [10.93, 105.31],  82: [10.35, 105.98],
  83: [10.25, 106.38],  84: [10.04, 105.76],  86: [10.38, 105.44],
  87: [10.03, 105.77],  89: [10.52, 105.12],  91: [10.03, 104.97],
  92: [10.13, 105.80],  93: [9.95,  106.34],  94: [9.60,  106.02],
  95: [9.27,  105.73],  96: [9.18,  105.15],
}

function jitter(seed: string, range = 0.12): number {
  let h = 5381
  for (let i = 0; i < seed.length; i++) h = ((h * 33) ^ seed.charCodeAt(i)) >>> 0
  return ((h % 1000) / 1000 - 0.5) * range * 2
}

export interface MapListing {
  id:         string
  slug:       string
  title:      string
  price_text: string | null
  province_id: number | null
}

interface ProvinceMapProps {
  provinceId:  number
  provinceName: string
  listings:    MapListing[]
}

function LeafletProvinceMap({ provinceId, provinceName, listings }: ProvinceMapProps) {
  const containerRef = useRef<HTMLDivElement>(null)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const mapRef = useRef<any>(null)

  useEffect(() => {
    if (!containerRef.current) return
    let mounted = true

    import('leaflet').then(mod => {
      if (!mounted || !containerRef.current) return
      const L = mod.default

      if (!document.getElementById('leaflet-css')) {
        const link = document.createElement('link')
        link.id   = 'leaflet-css'
        link.rel  = 'stylesheet'
        link.href = 'https://unpkg.com/leaflet@1.9.4/dist/leaflet.css'
        document.head.appendChild(link)
      }

      if (mapRef.current) { mapRef.current.remove(); mapRef.current = null }

      const coords = PROVINCE_COORDS[provinceId] ?? [16.5, 107.5]
      const map = L.map(containerRef.current, {
        center:             coords,
        zoom:               9,
        zoomControl:        true,
        attributionControl: false,
        scrollWheelZoom:    false,
      })

      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 18 }).addTo(map)

      L.control.attribution({ prefix: false, position: 'bottomright' })
        .addAttribution('© <a href="https://www.openstreetmap.org/copyright" target="_blank">OpenStreetMap</a>')
        .addTo(map)

      // Province centroid pin (larger)
      L.circleMarker(coords, {
        radius: 14, fillColor: '#1A4D2E', color: '#fff',
        weight: 3, opacity: 1, fillOpacity: 0.2,
      }).addTo(map)

      L.circleMarker(coords, {
        radius: 5, fillColor: '#1A4D2E', color: '#fff',
        weight: 2, opacity: 1, fillOpacity: 1,
      }).bindPopup(`<div style="font:600 13px/1.4 system-ui">${provinceName}</div>`).addTo(map)

      // Listing pins
      listings.slice(0, 40).forEach(l => {
        const lat = coords[0] + jitter(l.id + 'lat')
        const lng = coords[1] + jitter(l.id + 'lng')
        L.circleMarker([lat, lng], {
          radius: 7, fillColor: '#FF9500', color: '#fff',
          weight: 2, opacity: 1, fillOpacity: 0.85,
        })
          .bindPopup(
            `<div style="font:600 13px/1.4 system-ui;max-width:200px">
               <a href="/dat/${l.slug}" style="color:#1A4D2E;text-decoration:none">${l.title}</a>
               ${l.price_text ? `<div style="margin-top:4px;font-size:12px;color:#555">${l.price_text}</div>` : ''}
             </div>`,
          )
          .addTo(map)
      })

      mapRef.current = map
    })

    return () => {
      mounted = false
      if (mapRef.current) { mapRef.current.remove(); mapRef.current = null }
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [provinceId])

  return <div ref={containerRef} className="h-full w-full" aria-label={`Bản đồ đất nông nghiệp ${provinceName}`}/>
}

// Dynamic wrapper (ssr: false must be inside a Client Component)
const DynamicMap = dynamic(
  () => Promise.resolve(LeafletProvinceMap),
  { ssr: false, loading: () => <div className="h-full w-full animate-pulse bg-neutral-100"/> },
)

export function ProvinceMap(props: ProvinceMapProps) {
  return (
    <div className="h-[380px] overflow-hidden rounded-2xl border border-neutral-100 sm:h-[440px]">
      <DynamicMap {...props}/>
    </div>
  )
}
