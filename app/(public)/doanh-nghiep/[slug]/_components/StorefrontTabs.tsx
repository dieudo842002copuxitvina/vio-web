...
function ProductCard({ p }: { p: ProductRef }) {
  return (
    <Link
      ...
      className="flex h-full flex-col rounded-2xl bg-white p-4 shadow-[0_1px_6px_rgba(0,0,0,0.07)] no-underline transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98] dark:bg-[#2C2C2E]"
    >
      {p.is_featured && (
...
function ServiceCard({
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl bg-white p-4 shadow-[0_1px_6px_rgba(0,0,0,0.07)] dark:bg-[#2C2C2E]">
      ...
      <button
        ...
        onClick={() => onBook(s.id)}
        className={[
          'flex h-11 min-h-[44px] w-full items-center justify-center',
...
function NearbyCard({ n }: { n: NearbyRef }) {
  return (
    <Link
      ...
      className="flex items-center gap-3.5 rounded-2xl bg-white p-4 shadow-[0_1px_6px_rgba(0,0,0,0.07)] no-underline transition-colors hover:bg-gray-50 dark:bg-[#2C2C2E] dark:hover:bg-white/[0.05]"
    >
    ...
    <div className="rounded-2xl bg-white p-4 shadow-[0_1px_6px_rgba(0,0,0,0.07)] dark:bg-[#2C2C2E]">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gray-100 text-sm font-bold text-gray-600 dark:bg-gray-700 dark:text-gray-300">
...
export function StorefrontTabs({
  ...
  return (
    <>
      {/* ── Sticky tab bar ── */}
      <div className="sticky top-0 z-20 -mx-4 border-b border-gray-100 bg-white/90 px-4 backdrop-blur-xl dark:border-white/[0.06] dark:bg-[#1C1C1E]/90">
        <div className="flex gap-1">
          {tabs.map(tab => (
            <button
              ...
              className={[
                'flex h-11 min-h-[44px] items-center gap-1.5 border-b-2 px-4 text-[0.9375rem] font-semibold transition-colors',
                activeTab === tab.id
            ...
            {nearby.length > 0 && (
              <div>
                ...
                <ul className="grid grid-cols-1 gap-3 list-none m-0 p-0 sm:grid-cols-2">
                  {nearby.map(n => (
                    <li key={n.id}><NearbyCard n={n} /></li>
                  ))}
                </ul>
              </div>
            )}

            {!description && !addressText && products.length === 0 && nearby.length === 0 && (
              <p className="py-8 text-center text-[0.9375rem] text-gray-400">
